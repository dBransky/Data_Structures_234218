...
void HighTech::AddEmployee(int EmployeeId, int CompanyID, int Salary, int Grade)
{
    ...
    try
    {
        ...
        new_employee = new Employee(Salary, Grade, CompanyID, company);
...
void HighTech::RemoveEmployee(int employee_id)
{
    ...
    try
    {
        ...
        if (employee->GetCompany()->GetBestSalaryEmployee()->GetId() == employee_id)
        {
            ...
            best_earning_employees.insert(EmployeeByCompanyId(shared_ptr<Employee>(employee->GetCompany()->GetBestSalaryEmployee())),employee);
...
void HighTech::PromoteEmployee(int EmployeeID, int SalaryIncrease, int BumpGrade)
{
    ...
    try
    {
        ...
        Employee* best_emp = company->GetBestSalaryEmployee();
        if (best_emp->GetSalary() < employee->GetSalary() || (best_emp->GetSalary() == employee->GetSalary() && best_emp->GetId() > employee->GetId()))
        {
            employee->GetCompany()->GetCompanyEmployees().remove(SalaryId(employee->GetSalary(), EmployeeID));
            employee->GetCompany()->SetCompanyBestEmployee(
                    reinterpret_cast<Employee *>((employee->GetCompany()->GetCompanyEmployees().GetMaxId()).get()));
        }
        if (employee_with_best_salary->GetSalary() < employee->GetSalary() || (employee_with_best_salary->GetSalary() == employee->GetSalary() && employee_with_best_salary->GetId() > employee->GetId()))
        {
            best_earning_employees.remove(EmployeeByCompanyId(shared_ptr<Employee>(employee)));
            best_earning_employees.insert(EmployeeByCompanyId(shared_ptr<Employee>(employee->GetCompany()->GetBestSalaryEmployee())),employee);
            employee_with_best_salary = reinterpret_cast<Employee *>(best_earning_employees.GetMaxId().get());
...
void HighTech::GetAllEmployeesBySalary(int companyID, int **Employees, int *NumOfEmployees)
{
    ...
    else
    {
        try
        {
            ...
            Pair<Employee*, SalaryId>* pair_list = company->GetCompanyEmployees().GetFirstNum(company->GetAmountOfEmployees());
...
void HighTech::GetNumEmployeesMatching(int CompanyID, int MinEmployeeID, int MaxEmployeeID, int MinSalary, int MinGrade, int *TotalNumOfEmployees, int *NumOfEmployees)
{
    if (TotalNumOfEmployees == NULL || NumOfEmployees == NULL || CompanyID == 0 || MinEmployeeID <0 || MaxEmployeeID < 0 || MinSalary <0 || MinGrade < 0 || MinEmployeeID > MaxEmployeeID)
    {
        throw InvalidInput();
    }
    if (CompanyID < 0 && employee_with_best_salary == NULL)
    ...
    else
    {
        ...
        {
            Company *company = companies.find(CompanyID);
            Pair<Employee *, SalaryId> *pair_list = company->GetCompanyEmployees().GetObjectsFromKey(MinEmployeeID, MaxEmployeeID, TotalNumOfEmployees);
            int count = 0;
            for (int i = 0; i < *TotalNumOfEmployees; i++) {
                if (pair_list->element->GetSalary() >= MinSalary && pair_list->element->GetGrade() >= MinGrade) {
                    count++;
...
Employee::Employee(int salary, int grade, int company_id, Company *company) : salary(salary), grade(grade), company(company) {
